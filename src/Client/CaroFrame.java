...
public class CaroFrame extends JFrame implements ActionListener {
	/*private static CaroFrame instance = null;
	public static CaroFrame getInstance()
	{
		if(instance == null)
			instance = new CaroFrame();
		return instance;
	}*/
	private static final long serialVersionUID = 1L;
	private Integer width = CaroGraphics.width;
	private Integer height = CaroGraphics.height;
	...
	private JMenuBar createJMenuBar() {
		...
		String[] game = { "Tran choi moi", "Hiep moi", "", "Exit" };
		mb.add(createJMenu("Tro choi", game, KeyEvent.VK_T));
		String[] help = { "Huong dan", "", "Gioi thieu" };
		mb.add(createJMenu("Huong dan", help, KeyEvent.VK_H));
		return mb;
	}
	...
	private JPanel createPanelStatus(boolean player) {
		...
		if (player) {
			...
			lbNamePlayerO = new JLabel("Người choi2");
	...
	private JPanel createPanelBottom(boolean player) {
		String[] str1 = { "Undo", "Xin thua" };
		String[] str2 = { "TrÃ² chÆ¡i má»›i", "Hiá»‡p má»›i" };
		String[] str;
		if (player) {
			str = str1;
		} else {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (command == "Xin thua") {
			//actionGiveIn();
			//client.sendMessage(new ChatMessage(ChatMessage.MESSAGE, "Hello"));
		}
	}

	private void actionNewGame() {
		int select = showDialog("CÃ¡c báº¡n thá»±c sá»± muá»‘n táº¡o trÃ² chÆ¡i má»›i?",
				"TrÃ² chÆ¡i má»›i");
		if (select == 0) {
			scoreO = 0;
			scoreX = 0;
	...
	private void actionNewUnit() {
		int select = showDialog("CÃ¡c báº¡n thá»±c sá»± muá»‘n táº¡o hiá»‡p má»›i?",
				"Hiá»‡p má»›i");
		if (select == 0) {
			clear();
		}
	...
	private void actionExit() {
		int select = showDialog("CÃ¡c báº¡n thá»±c sá»± muá»‘n thoÃ¡t?", "ThoÃ¡t");
		if (select == 0) {
			System.exit(0);
		}
	}
	...
	private void actionGiveIn() {
		...
		int select = showDialog(playerName + " thá»±c sá»± muá»‘n xin thua?",
				"TrÃ² chÆ¡i má»›i");
		if (select == 0) {
			scoreO += sO;
			scoreX += sX;
	...
	private void win(int winer) {
		...
		Object[] options = { "TrÃ² chÆ¡i má»›i", "Hiá»‡p má»›i", "ThoÃ¡t" };
		int select = JOptionPane.showOptionDialog(this, "ChÃºc má»«ng "
				+ playerName + " Ä‘Ã£ chiáº¿n tháº¯ng trong hiá»‡p Ä‘áº¥u "
